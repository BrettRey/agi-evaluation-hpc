...
def plot_published_comparison(results: pd.DataFrame, path: Path) -> None:
    ...
    specs = [
        ...
        ("ins_adjusted", "Pseudo-null-adjusted INS"),
        ("wtd_adjusted", "Pseudo-null-adjusted WTD"),
    ]
...
def plot_wtd_estimators(results: pd.DataFrame, path: Path) -> None:
    ...
    ax.scatter(x, ordered["wtd_adjusted"], label="pseudo-null-adjusted", color="#4C72B0", s=20)
...
def plot_sensitivity(sensitivity: pd.DataFrame, full_results: pd.DataFrame, path: Path) -> None:
    ...
    axes[0].plot(agg["n_trials"], agg["ins_adjusted"], marker="o", label="pseudo-null-adjusted INS", color="#4C72B0")
    ...
    axes[1].plot(agg["n_trials"], agg["wtd_adjusted"], marker="o", label="pseudo-null-adjusted WTD", color="#4C72B0")
    axes[1].plot(agg["n_trials"], agg["wtd_split"], marker="o", label="split WTD", color="#55A868")
    axes[1].plot(agg["n_trials"], agg["absolute_tail"], marker="o", label="absolute loss tail", color="#8172B2")
...
def main() -> None:
    ...
    metadata = {
        ...
        "limitations": [
            "Subtracting the baseline pseudo-null expectation is a noise correction, not an unbiased non-null effect estimator.",
            "Split WTD is unbiased for a noisy-selected set and may miss the oracle latent tail.",
            "Absolute loss tail and WTD answer different questions and aren't combined.",
