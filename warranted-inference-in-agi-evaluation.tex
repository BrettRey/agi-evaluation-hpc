...
A running example makes the issue concrete. A law firm is deciding whether to release a legal-research assistant built on a model whose developer publishes strong scores across a multidomain battery and describes it as broadly capable. Such claims can outrun delivered reliability. An independent evaluation of the leading commercial legal-research assistants found them fabricating authority between 17\% and 33\% of the time, against vendor assurances of hallucination-free citation \citep{magesh2025legalhallucination}. The firm needs to know what those scores tell it about its own next model--retrieval--tool workflow, on local matters, when used by its lawyers, with clients and courts bearing the consequences. A law firm makes a fitting decider, since judging what an authoritative-seeming expert claim is actually good for, rather than deferring to its confident presentation, is work the law has long done with expert evidence \citep[ch.~16]{coady1992testimony}. The published battery is the source; the firm's later workflow and matters are the target.
...
Within this architecture, \term{projectibility} is assessed for each source--target link: the evidential support that observations and revisable background assumptions give a declared link, relative to a proposed use. The law firm's link runs from the developer's multidomain scores to its own matters, and its projectibility is the support those scores and the firm's own knowledge give the claim that they carry to its work. That support is graded and relative to the use, not a single number or a yes-or-no verdict: the same scores may back a claim about routine matters yet fail for a novel practice area. It's the standing of that one link, not the warrant of the firm's whole release decision, which also turns on values and alternatives. A \term{projectibility audit} names each link, states the facet the claim has to cross~-- here, from the developer's tested tasks to the firm's matters~-- matches evidence to it, assigns who supplies that evidence, and blocks a run of passed benchmarks from adding up to a claim of general capability.

For confirmatory use, the audit begins with a prospective declaration. If the firm forms its claim after seeing the developer's trial results, it can still test the claim on fresh matters, but those results can't confirm it. Its target also has to differ from the source. Testing the tool only on tasks shaped like the benchmark's own re-checks what the benchmark already measured, not whether the tool handles the firm's actual matters. Generalization within the tested universe stays a genuine inference.

Prospective declaration narrows discretion without eliminating it. Even with the firm's claim fixed in advance, its analyst still chooses which practice areas to pool, which tail to read, and which matters count~-- choices the declaration doesn't fully pin down~-- so multiple comparisons can still distort the result \citep{GelmanLoken2013}. A declaration makes that remaining discretion inspectable; it doesn't remove it.

The audit keeps four questions apart. For the firm, the claim is that the scores carry to its matters; the evidence is a test on held-out matters; the regularity is whatever makes the tool's competence hold up across matters; and the explanation, if any, is the mechanism behind it. Projectibility is the claim's standing across all four, not the warrant alone. The firm can confirm the tool holds without learning why, and knowing why wouldn't by itself extend the claim to a new practice area.

The firm's release decision adds another layer. The projectible premise about the tool has to be combined with feasible alternatives such as restricted use or mandatory review, the firm's constraints and authority, its values, and a release rule. Those values enter earlier too, in what the firm's rubric counts as a good answer, not only at the moment of release \citep[747--749]{messick1995Validity}.

Projectibility covers only the beyond-observation links in the larger validity argument. \Textcite[741--747]{messick1995Validity} asks five more questions the firm still faces: whether the benchmark's content represents its matters, whether the tool's responses engage real legal reasoning, whether the scoring fits the construct, whether its scores relate to outside criteria, and whether the consequences bear on release. No single holdout answers all of these. Nor are the links interchangeable: reaching more of the firm's matters, a new practice area, the next model version, and next year's performance each need different evidence.
...
Predictive usefulness doesn't by itself make a collection of predictors a coherent score. Aggregation adds questions about meaning and tradeoffs. For the developer's battery, \term{commensurability} asks whether a point gained on one cognitive domain and a point gained on another can be added with the meaning the firm needs; \term{compensability} asks whether strength on one domain may offset weakness on another for the firm's matters; and \term{projectibility} asks whether the resulting total supports the claim that the tool carries to those matters. \Textcite[745--747]{messick1995Validity} treats the choice between compensatory totals, profiles, and minimum requirements as part of asking whether a score's structure matches its intended interpretation. Because a linear factor model of the kind underlying CHC and g builds in compensation \citep[632]{carroll1993human}, reading the developer's total as a measure already commits the firm to trading strength on one domain against weakness on another~-- even when a matter turns on the very domain a high score elsewhere conceals.
