...
A reanalysis of 32 released model--benchmark comparisons reproduces the published paired-condition quantities, and simulations in which the true values are fixed by design show that a stable average can coexist with a large change tail and with high absolute case risk. These analyses distinguish among descriptive quantities and show that their estimators target different objects; they don't show that any of them predicts an untested target. Because developers can't validate every open-ended use, the framework divides evidential labour: developers characterize bounded source evaluations; deployers complete and monitor warrants for local use.
\end{abstract}
...
A running example makes the issue concrete. A law firm deciding whether to release a legal-research assistant needs to know what a developer's benchmark result tells it about its own next model--retrieval--tool workflow, on local matters, when used by its lawyers, with clients and courts bearing the consequences. The developer's benchmark is the source; the firm's later workflow and matters are the target.
...
\item \textbf{Evidential standard, failure conditions, and responsibility}: the evidence and tolerance that could support the claim, what would defeat or restrict it, what would leave it unresolved, who supplies the source evidence, and who can perform target-specific testing
\end{enumerate}
...
For a predictive projection, each row of the analysis should represent the independent unit fixed by the declared source--target link. An item-generalization study may reserve task-template clusters as the independent units for final testing. An extrapolation study needs an external task or episode sample from the target domain rather than more items from the same universe. A study of projection across perturbations may compare entire held-out families within systems. A model-population study needs enough independent lineages to hold out lineages rather than outputs. A study of transport across workflows or sites holds out whole sites or teams, not operators within one site. A temporal study requires a chronological split. Response replicates estimate within-unit quantities; they don't multiply the number of independent systems, items, or contexts.
...
CHC theory \citep{carroll1993human,mcgrew2009chc,schneider2018chc} can enter AI evaluation in four roles: a content taxonomy; a factor structure, or account of which task scores vary together, in a declared system population; an input to external prediction; or a functional or causal interpretation. Evidence for one role doesn't establish the next. The framework proposed by \textcite{hendrycks2025agi} principally supplies the first; the other claims require additional study.
